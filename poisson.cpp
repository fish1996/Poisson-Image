...
void Poisson::multi_light()
{
	for (int i = 1; i < gradient->rows-1; i++) {
		for (int j = 1; j < gradient->cols-1; j++) {
			...
			cal_light(gradient->at<Vec3f>(i, j)[0], magnitude->at<Vec3f>(i, j)[0]);
			cal_light(gradient->at<Vec3f>(i, j)[1], magnitude->at<Vec3f>(i, j)[1]);
			cal_light(gradient->at<Vec3f>(i, j)[2], magnitude->at<Vec3f>(i, j)[2]);
		}
	}
	patchNaNs(*gradient);
}
...
{
	magnitude->at<Vec3f>(i, j)[k] = sqrt(pow2(tmp.at<Vec3f>(i - 1, j)[k]
		+ tmp.at<Vec3f>(i + 1, j)[k] - 2 * tmp.at<Vec3f>(i, j)[k])
		+ pow2(tmp.at<Vec3f>(i, j - 1)[k]
			+ tmp.at<Vec3f>(i, j + 1)[k] - 2 * tmp.at<Vec3f>(i, j)[k]));
}
